...
**Status:** **⏳** proposed — phased milestones locked in chat (GTK hello → embed → a11y)

**Where we are:**

- **✔️** Phase 0 — docs / this plan
- **✔️** Phase 1 — GTK Hello World on Android (**no** WebView) — await user **✅** on device
- **✔️** Phase 2 — basic WebView embed + display — APK builds/installs; await user **✅** (roojs.com visible)
- **⏳** Phase 3 — accessibility dump / fill / press
...
- ℹ️ Snappr host patterns (load settle / cookies / CF): `/home/alan/gitlive/app.Snappr2` — browse only; **do not** port CaptureEngine
...
- **🚫** Screenshot / capture / PDF / `get_snapshot` / Snappr CaptureEngine — **out of scope** for this library (that is Snappr product work, not OLLMchat browser-tool work).
...
- **ℹ️** OLLMchat **5.0.2** currently assumes “Pixiewood / JNI as needed” inside chat — better to land the hard embedding + a11y bridge **here**, then wire `#if ANDROID` in `libocwebkit` like Snappr’s `#if WINDOWS`.
...
- **🚫** Porting Snappr screenshot / crawl-pool / PMAN / SERP JS.
...
Match what **Snappr / `libocwebkit.Browser`** actually call on `WebView` today — not the entire WebKitGTK docs.
...
- **🔷** `get_network_session()` → cookie manager (`get_cookies` / `add_cookie` async, accept policy) — enough for Snappr-style `site_cookies`
- **🔷** Android-only: `ready` (native WebView attached), like WebView2Gtk

### Nice / when needed by CF

- **💩** `main_document_response (status, headers)` — Snappr/OLLMchat Cloudflare watches this on Linux/Windows; Android may need a `WebViewClient` / request interceptor equivalent
...
- **ℹ️** Chromium returns **null** from `getAccessibilityNodeProvider` when content is marked **obscured by another view**. Freeze overlays / GTK chrome on top of the WebView may **kill** the tree while shown — spike interaction with Snappr-style freeze mask.
...
- **⏳** **🔷** Freeze overlay / obscured-view killing `getAccessibilityNodeProvider`.
...
- **⏳** **🔷** Globe / CF / cookies in **5.0.2** once embed + a11y dump/press work in the demo.
...
- **💩** Cookies / `NetworkSession` / CF `main_document_response` — defer after Phase 2 **✅**
- **🚫** No a11y dump/fill/press in this phase
- **🚫** No snapshot / capture
- **⏳** User **✅** on device — unlock phone and confirm roojs.com renders over GTK chrome
...
- **⏳** **🔷** Force-on path (see research) — fail the phase if dump only works with TalkBack
- **⏳** **🔷** Structured tree dump + fill + press APIs for `libocwebkit` adapter
- **⏳** **🔷** Example that prints dump after load (roojs.com / simple form page)
- **⏳** **🔷** Document route/id scheme + any WebView-version pitfalls
- **🚫** No DOM/JS scrape fallback — stop and update the plan if a11y wake fails
- **🚫** Do not bake final OLLMchat markdown here unless useful for debugging
...
- **⏳** **💩** Full WebKitGTK-shaped cookie / settle surface for Snappr-style `Browser`
- **⏳** **💩** VAPI + pkg-config polish / install layout for OLLMchat subproject
