...
- **💩** `main_document_response (status, headers)` — OLLMchat Cloudflare watches this on Linux/Windows; Android may need a `WebViewClient` / request interceptor equivalent
...
- **⏳** **💩** Full settle / CF `main_document_response` for OLLMchat-style `Browser`
